...
void add_global_monitor(int writefd, int readfd) {
    pid_t pid = getpid();
...
/*
// delete info with this specific monitor from global struct
void delete_global_monitor(pid_t pid) {
    Listnode *listnodeprev, *listnodecurr;
    globalData *gdata;

    listnodecurr = gMonitor.globalDataList.head;
    gdata = (globalData *) listnodecurr->item;
    if(gMonitor.globalDataList.size == 1 && gdata->pid==pid) {
        free(listnodecurr);
        gMonitor.globalDataList.head = NULL;
        return;
    }

    while(listnodecurr!=NULL && gdata->pid!=pid) {
        listnodeprev = listnodecurr;
        listnodecurr = listnodecurr->next;
        if(listnodecurr!=NULL)
            gdata = (globalData *) listnodecurr->item;
    }
    ...
    listnodeprev->next = listnodecurr->next;
    free(listnodecurr);
    return;            
}*/
...
int send_country_parent() {
    ...
    if(pipe_write(gdata->writefd,buf,strlen(buf),'d') == -1) 
        return -1;
    return 0;
}