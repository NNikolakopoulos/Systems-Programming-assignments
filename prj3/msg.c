...
int initialize_monitors(int *fds, char *input_dir, int numMonitors) {
    ...
    */
    ...
    while ((direntp = readdir(dirp)) != NULL)
    {
        ...
        if(pipe_write(fds[i],package,strlen(package),'2') == -1) 
            return -1;
...
int ready_to_exit(int *fds, int numMonitors) {
    ...
    for(i=0; i<numMonitors; i++) {
        if(pipe_write(fds[i],"",1,'9') == -1)  // assign protocol '9' to tell monitors to exit
            return -1;
    }
}

int quit(int *fds,int numMonitors, char *input_dir) {
    for(int i=0; i<numMonitors; i++) {
        if(pipe_write(fds[i],"",0,'9') == -1)  // assign protocol '9' to tell monitors to exit
            return -1;
    }
  ...
  for (int i = 0; i < numMonitors; i++)  // Wait for monitors to terminate
    ...
    while ((direntp = readdir(dirp)) != NULL)
    {
        if (!strcmp(direntp->d_name, ".") || !strcmp(direntp->d_name, "..") || direntp->d_ino == 0)  // Ignore . and .. dirs
            continue;
        unsigned long buf_length = strlen(direntp->d_name) + 2;
...
int send_travelRequest(int *fds,char **args, int monitor_i) {
    ...
    if(pipe_write(fds[monitor_i],pipe_buf,pipe_buf_size,'5') == -1)  // assign protocol '5' to send monitors travel request
            return -1;
...
int send_request_response(int *fds, List *list, int monitor_i, char *date, char *country, char *virusname, bool accepted) {
    char *pipe_buf;
    ...
    if(pipe_write(fds[monitor_i],pipe_buf,strlen(pipe_buf),'a') == -1)  // assign protocol 'a' to send travel request stats to monitor
        return -1;
...
int send_search_status(int *pipesfds,char **args, int numMonitors) {
    ...
    for(i=0; i<numMonitors; i++) {
        if(pipe_write(pipesfds[i],pipe_buf,pipe_buf_length,'b') == -1)  // assign protocol 'b', for this job
            return -1;
    }
    fd_set readfds;
    int countMonitors=0;
    while(1) {
        FD_ZERO(&readfds);
        for(i=0; i<numMonitors; i++) 
            FD_SET(pipesfds[i],&readfds);
        
        int retval = select(FD_SETSIZE,&readfds,NULL,NULL,NULL);
        for(i=0; i<numMonitors; i++) {
            if(FD_ISSET(pipesfds[i],&readfds)) {                            
                pipe_buf = pipe_read(pipesfds[i]);
...
int handle_addVaccinationRecords(int sock_fd, char *country, List *list, int monitor_i, struct sockaddr_in *saddr) {
    
    if(pipe_write(sock_fd,country,strlen(country),'2') == -1) {
        perror("error while sending country to monitor for addVaccinationRecord");
        exit(1);
    }

    if(pipe_write(sock_fd,"",0,'4') == -1) { // tell monitors that parent is ready to get bloom filters
        perror("error while sending ready for bloom to monitor for addVaccinationRecord");
        exit(1);
    }
