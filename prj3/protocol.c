...
void set_is_setup() { is_setup = true; }
void set_not_is_setup() { is_setup = false; }
...
int code_4(HThash *hash, virusList *list, int writefd);
...
int code_4(HThash *hash, virusList *list, int writefd) {
    if(!*(hash->changed) && is_setup) {//if no changed occured , then 
...
int code_a(char *msg,unsigned long msg_length) {
    ...
    add_country_global_struct(country);
...
int code_d(char *msg, unsigned long msg_length, int *sock_fds, int monitor_i, List *list) {
    char *token, *rest;
    rest = msg;
    while(token = strtok_r(rest," ",&rest)) {
            if(pipe_write(sock_fds[monitor_i],token,strlen(token),'2') == -1) {
            perror("error while sending country to monitor for addVaccinationRecord");
            exit(1);
        }
    }
    
    if(pipe_write(sock_fds[monitor_i],"",0,'4') == -1) { // tell monitors that parent is ready to get bloom filters
