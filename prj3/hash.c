...
HThash HTcreate()
{
    ...
    hash.changed = calloc(1,sizeof(bool));
    *(hash.changed)=false;
...
HThash HTinsertAll(HThash hash, virusList *viruslist,char *fname)
{
    ...
    while( getline(&line, &n, fp) != -1)  {
        ...
        if(token!=NULL) {
            ...
            char *daystr=NULL,*monthstr=NULL,*yearstr=NULL, *tempstr, *newstring;
            daystr=strtok(token,"\n -");
            //daystr = strdup(tempstr);
            monthstr=strtok(NULL,"\n -");
            //monthstr = strdup(tempstr);
            yearstr=strtok(NULL,"\n -");
            //yearstr = strdup(tempstr);
            yearstr[4] = '\0';

            rec->dateVaccinated = (char *) calloc(12,sizeof(char));
            snprintf(rec->dateVaccinated,11,"%s-%s-%s",yearstr,monthstr,daystr);
        }
        else 
