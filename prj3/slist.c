...
char *skipListSearchTravelReq(skipList *list, char *citizenID, char *date1)
{
    ...
    if(list->size!=1 && (strcmp(temp->key,citizenID)!=0) )
        temp=temp->arr[0];
    if(temp!=NULL && !strcmp(temp->key,citizenID)) {
        rec = (Record *) temp->item;
        char *lastmonths = last_months(rec->dateVaccinated);
        lastmonths = reverse_date(lastmonths);
        if(rec->vaccinated && (strcmp(date1, lastmonths)>0))  // if he is vaccinated in these months return 1
            return lastmonths;
        if(rec->vaccinated)
            return lastmonths;
    }
